...
async def weekly_parent_report(app):
    ...
    async for doc in _batch_stream_users(batch_size=500):
        ...
        try:
            ...
            logger.exception(
                "Parent report generation failed for %s",
                safe_user_ref(user.get("telegram_id")),
            )
...
async def post_init(application):
    ...
    if os.getenv("RAILWAY_ENVIRONMENT") or WEBHOOK_URL:
        logger.info(
            "🔧 Railway/webhook mode detected — scheduler will be started by server.py."
        )
        return
    ...
    scheduler.add_job(
        panic_reminder, "cron", hour=9, minute=0, args=[application], id="panic_noon"
    )
    scheduler.add_job(
        panic_reminder,
        "cron",
    ...
    scheduler.add_job(
        db.check_and_expire_subscriptions, "interval", hours=1, args=[application]
    )
    scheduler.start()
...
def build_app():
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            CONFESSION_BOX: [
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND, _guard(handle_confession)
                )
            ],
            BOSS_FIGHT: [
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND, _guard(handle_boss_answer)
                )
            ],
            AWAITING_TELEBIRR_TX: [
                MessageHandler(
            ...
            AWAITING_FEATURE_SUGGESTION: [
                MessageHandler(
                    filters.TEXT & ~filters.COMMAND, _guard(handle_suggestion)
                )
            ],
        },
        fallbacks=[
    ...
    async def safe_log_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.message and update.message.text:
            ...
            logger.info(
                "[BTN] user=%s data=%s", user_ref, str(update.callback_query.data)[:80]
            )
    ...
    for cmd in [
        ...
        handler = (
            locals().get(f"cmd_{cmd}")
            or globals().get(f"cmd_{cmd}")
            or (start if cmd in ["start", "menu"] else None)
        )
        if handler:
...
def main():
    ...
    uvicorn.run(
        web_app, host="0.0.0.0", port=port, proxy_headers=True, forwarded_allow_ips="*"
    )
