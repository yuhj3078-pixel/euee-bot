...
def get_random_real_question(subject: str) -> dict | None:
    try:
        ...
        response = (
            supabase.table("real_exam_questions")
            .select("*")
            .eq("subject", subject)
            .limit(20)
            .execute()
...
def is_subscription_active(telegram_id: int) -> bool:
    ...
    if not expires:
        return True  # Free tier doesn't expire
...
def get_leaderboard(limit: int = 10) -> list:
    try:
        ...
        response = (
            supabase.table("users")
            .select("name, questions_total, streak")
            .order("questions_total", desc=True)
            .limit(limit)
            .execute()
        )
...
def get_battle(battle_id: str) -> dict | None:
    try:
        supabase = _get_supabase()
        resp = (
            supabase.table("battles").select("*").eq("battle_id", battle_id).execute()
        )
...
def join_battle(battle_id: str, user2_id: int) -> dict | None:
    try:
        ...
        status = (
            "active"
            if battle.get("challenger_id") != user2_id
            else battle.get("status", "waiting")
        )
        updates = {
            "opponent_id": user2_id
            if battle.get("challenger_id") != user2_id
            else battle.get("opponent_id"),
            "status": status,
        }
...
def submit_battle_answer(
    battle_id: str, user_id: int, answer: str, time_taken: float, is_correct: bool
) -> dict | None:
    try:
        battle = get_battle(battle_id)
        if not battle:
        ...
        _get_supabase().table("battles").update(updates).eq(
            "battle_id", battle_id
        ).execute()
        return get_battle(battle_id)
    except Exception as e:
...
def check_transaction_exists(tx_id: str) -> bool:
    try:
        supabase = _get_supabase()
        resp = (
            supabase.table("payment_attempts")
            .select("tx_id")
            .eq("tx_id", tx_id)
            .execute()
        )
...
def save_payment_attempt(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.info(f"Payment attempt saved: tx_id={tx_id}, result={result.data}")
        return True
    except Exception as e:
        logger.exception(f"Error saving payment attempt: {e}")
        return False
...
class BatchRef:
    ...
    def commit(self):
        ...
        for op, ref, data in self.ops:
            if op == "set":
                supabase.table(ref.collection).upsert(data).execute()
            elif op == "update":
                supabase.table(ref.collection).update(data).eq(
                    ref.id_field, ref.id_val
                ).execute()
...
def approve_payment(tx_id: str) -> bool:
    try:
        ...
        # Get payment record
        resp = (
            supabase.table("payment_attempts").select("*").eq("tx_id", tx_id).execute()
        )
        if not resp.data:
            return False
        payment = resp.data[0]
        ...
        supabase = _get_supabase()
        supabase.table("payment_attempts").update({"status": "APPROVED"}).eq(
            "tx_id", tx_id
        ).execute()
        return True
    except Exception:
...
def reject_payment(tx_id: str) -> bool:
    try:
        supabase = _get_supabase()
        supabase.table("payment_attempts").update({"status": "REJECTED"}).eq(
            "tx_id", tx_id
        ).execute()
        return True
    except Exception:
...
def get_user_by_parent_token(token: str) -> dict | None:
    try:
        supabase = _get_supabase()
        response = (
            supabase.table("users").select("*").eq("parent_token", token).execute()
        )
        return response.data[0] if response.data else None
    except Exception:
        return None
    ...
    try:
        if not is_correct:
            ...
            try:
                ...
                logger.warning(
                    f"Could not save wrong question (table might not exist): {e}"
                )
        ...
        score_by_subject[subject] = score_by_subject.get(subject, 0) + (
            1 if is_correct else 0
        )
        subject_correct[subject] = subject_correct.get(subject, 0) + (
            1 if is_correct else 0
        )
        subject_wrong[subject] = subject_wrong.get(subject, 0) + (
        ...
        try:
            ...
            logger.exception(
                "Failed to update user aggregates for %s", safe_user_ref(telegram_id)
            )
    except Exception as e:
...
def get_wrong_questions(telegram_id: int, subject: str = None, limit: int = 20) -> list:
    try:
        supabase = _get_supabase()
        query = (
            supabase.table("wrong_questions").select("*").eq("telegram_id", telegram_id)
        )
        if subject:
...
def clear_wrong_questions(telegram_id: int, subject: str = None):
    try:
        supabase = _get_supabase()
        query = (
            supabase.table("wrong_questions").delete().eq("telegram_id", telegram_id)
        )
        if subject:
            query = query.eq("subject", subject)
        query.execute()
...
def add_feature_suggestion(telegram_id: int, suggestion: str):
    try:
        ...
        supabase.table("feature_suggestions").insert(
            {"telegram_id": telegram_id, "suggestion": suggestion}
        ).execute()
    except Exception:
        pass


def save_feature_suggestion(
    telegram_id: int,
    username: str = "",
    text: str = "",
    suggestion: str = "",
...
def check_feature_rate_limit(
    telegram_id: int, feature_name: str, hours: int = 24
) -> bool:
    try:
        user = get_user(telegram_id)
        if not user:
...
def complete_boss_fight(telegram_id: int):
    try:
        ...
        supabase = _get_supabase()
        supabase.table("boss_fights").update({"completers": list(completers)}).eq(
            "id", boss["id"]
        ).execute()
...
def get_cached_content(key: str) -> dict | None:
    try:
        supabase = _get_supabase()
        resp = (
            supabase.table("content_cache").select("*").eq("cache_key", key).execute()
        )
...
def set_cached_content(key: str, data: dict):
    try:
        ...
        supabase.table("content_cache").upsert(
            {"cache_key": key, "data": data, "updated_at": _now()}
        ).execute()
    except Exception:
        pass
...
def clear_subject_notes_cache(subject: str):
    try:
        supabase = _get_supabase()
        supabase.table("content_cache").delete().ilike(
            "cache_key", f"%{subject}%"
        ).execute()
    except Exception:
        pass
...
def check_and_expire_subscriptions(bot_app=None):
    ...
    try:
        ...
        for u in expired_users:
            ...
            update_user(
                tid,
                {"tier": "free", "subscription_active": False, "tier_updated_at": now},
            )
            # Log the change
            ...
            logger.info(
                "Subscription expired for %s (was %s)", safe_user_ref(tid), old_tier
            )
